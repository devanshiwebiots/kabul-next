...
import { CommonFileUploadProp } from "@/Types/Common";

const CommonFileUpload: React.FC<CommonFileUploadProp> = ({ maxFiles, multiple, body, className }) => {
  ...
  return (
    <CardBody>
      <Form>
        <Dropzone onChange={updateFiles} value={extFiles} maxFiles={maxFiles} multiple={multiple} uploadConfig={{ url: BASE_URL + "/file" }} header={false} footer={false} minHeight={body ? "180px" : "80px"} className={`dropzone dz-clickable ${className ? className : ""}`}>
