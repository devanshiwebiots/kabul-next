...
import { LightThemeList, LightThemeLists, LinkColorData } from "@/Data/UiKits/Alert";
          ...
          <Row>
            ...
            <Col xl={6}>
              {LightThemeLists?.map(({ title, color, className }, index) => (
                <Fragment key={index}>
                  <p className="mb-0 f-w-500">{title} Light Alert</p>
                  <Alert color={color}>
                    <p className={className}>This is a <Link className={`alert-link text-lowercase ${className}`} href={Href}>{title} alert</Link> with an example link. Check it out.
                    </p>
                  </Alert>
                </Fragment>
              ))}
