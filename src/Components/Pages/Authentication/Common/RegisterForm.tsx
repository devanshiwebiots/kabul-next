...
import { AgreeWithSignUp, AlreadyHaveAnAccount, AuthSignIn, CreateAccount, CreateYourAccount, EmailAddressSignUp, EnterYourPersonalDetailsToCreateAccount, Href, OrSignUpWith, PasswordSignUp, PrivacyPolicy, YourNameSignUp } from "@/Constant";
import { SignupSubmitProp } from "@/Types/Auth";
...
const RegisterForm = () => {
  ...
  const submitHandler = (values: SignupSubmitProp, { resetForm }: { resetForm: VoidFunction }) => resetForm();
        ...
        <Formik initialValues={{firstname: "", lastname: "", email: "", password: "" }} onSubmit={submitHandler}>
          {() => (
            <Form className="theme-form">
              ...
              <SocialLinks />
              <p className="mt-4 mb-0 text-center">{AlreadyHaveAnAccount}<Link className="ms-2" href={Routes.Login}>{AuthSignIn}</Link></p>
            </Form>
          )}
        </Formik>
      </div>
