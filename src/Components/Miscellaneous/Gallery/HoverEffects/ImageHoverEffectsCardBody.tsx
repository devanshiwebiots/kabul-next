        ...
        <Gallery>
          {
            HoverImageDataList?.map((item, index) => (
              <figure className={`mx-0 col-md-3 col-6 img-hover hover-${props.data}`} key={index}>
                <Item original={`${ImagePath}/lightgallry/0${item}.jpg`} width='1024' height='768'>
                  {({ ref, open }) => (
                    <Link href={Href} onClick={open}>
                      <div className='overflow-hidden'>
                        <img ref={ref as unknown as React.MutableRefObject<HTMLImageElement>} src={`${ImagePath}/lightgallry/0${item}.jpg`} alt='' className='w-100 h-100' />
                      </div>
                    </Link>
                  )}
                </Item>
