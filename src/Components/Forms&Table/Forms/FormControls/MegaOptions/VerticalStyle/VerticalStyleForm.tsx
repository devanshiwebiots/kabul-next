...
import { VerticalStyleFormProp } from "@/Types/Forms";
      ...
      <Row>
        {VerticalStyleDataList?.map(({ megaTitle, child }, index) => (
          <Fragment key={index}>
            ...
            {child?.map(({ id, color, cardClass, name, badgeTitle, digits, spanText, check, spanClass, star }: VerticalStyleFormProp) => (
              <Col sm={6} key={id}>
                      ...
                      <Label className="mb-0 w-100" htmlFor={`radios${id}`} check>
                        <span className="flex-grow-1 megaoption-space">
                          ...
                          <span className={spanClass}>
                            {star && star.map(({ id, starClass }) => <i key={id} className={`icofont icofont-star txt-warning ${starClass}`}></i>)}
                            {spanText}
                          </span>
                        </span>
                      </Label>
