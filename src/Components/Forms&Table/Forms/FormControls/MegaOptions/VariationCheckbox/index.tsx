          ...
          <Row className="g-3">
            <Col xl={4} md={5}>
              <div className="card-wrapper border rounded-3 h-100 checkbox-checked">
                ...
                {VariationCheckboxDataList?.map(({ color, check, labelText }, index) => (
                  <div className="payment-wrapper" key={index}>
                    <div className="payment-first">
                      <FormGroup className={`checkbox checkbox-${color}`} check>
                        <Input id={`check-${index + 1}`} type="checkbox" defaultChecked={check} />
                        <Label className="mb-0" htmlFor={`check-${index + 1}`} check>{labelText}</Label>
                      </FormGroup>
                    </div>
                  </div>
                ))}
