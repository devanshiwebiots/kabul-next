        ...
        <Col sm={6} key={index}>
          <div className="card-wrapper border rounded-3 h-100 light-card">
            <FormGroup check className="radio radio-primary">
              <Input id={data.value} type="radio" name="shippingMethod" value={data.value} defaultChecked={shippingMethod === data.value ? true : false} onChange={getUserData} />
              <Label className="mb-0" htmlFor={data.value} check>{data.label}</Label>
            </FormGroup>
            <p className="p-0">Estimated {data.details} days shipping</p>
          </div>
        </Col>
      ))}
